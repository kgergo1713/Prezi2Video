# Hasznalati utasitas — Prezi2Video

Ez a program egy Google Slides prezentaciobol automatikusan keszit egy TV-re alkalmas, vegtelenul ismetlodo videofajlt.

---

## Amire szukseg van

- Windows 10 vagy ujabb
- Internetkapcsolat (csak az elso futasnal, kb. 5-10 perc letoltes)
- A prezentacio exportalva PDF formatumban (lasd alabb)

---

## 1. lepes: Prezentacio exportalasa PDF-kent

1. Nyisd meg a prezentaciot **Google Slides**-ban.
2. Kattints a menuben: **Fajl → Letoltes → PDF dokumentum (.pdf)**
3. A letoltott fajlt helyezd ugyanabba a mappaba, ahol a `konverter.bat` talalhato.

> A program automatikusan megkeresi a `V1_Master_Template.pptx.pdf` nevu fajlt.
> Ha mas neven mentetted el a PDF-et, huzd ra egyszeruen a `konverter.bat` ikonra.

---

## 2. lepes: Konvertales inditasa

Kattints duplat a **`konverter.bat`** fajlra.

Megnyilik egy fekete ablak, amely vegigkoveti a folyamatot:

- Elso futasnal letolti a szukseges programokat (egyszer, utana nem kell internet).
- A PDF-bol kepeket keszit.
- A kepekbol osszeallitja a videot (nagy prezentacionalnal ez toebb percet is igenybe vehet).
- Elkeszul a **`*_tv.mp4`** fajl ugyanabban a mappaban, ahol a PDF van.

Az ablak vegul bezarul. Ha piros hibauzenetet latsz, keresz meg minket (elerhetseg lent).

---

## 3. lepes: Masolj USB-re

1. Dugj be egy **USB-pendraivet** a szamitogepbe.
2. Masold at a keszult **`*_tv.mp4`** fajlt a pendrive-ra.
3. Hugd ki a pendrive-ot a gepbol.

---

## 4. lepes: TV-n lejatszas

1. Dugd be a pendrive-ot a **Samsung TV** USB-portjaba.
2. A taviranityon: **Source (Forras) → USB**
3. Keresd meg a `.mp4` fajlt es nyisd meg.

A video automatikusan vegtelenul ismetlodik (a ismetles be van sutve a fajlba, nem kell a TV menujet allitani).

---

## Kerdesek, problemak

E-mail: [kgergo1713@gmail.com](mailto:kgergo1713@gmail.com)
